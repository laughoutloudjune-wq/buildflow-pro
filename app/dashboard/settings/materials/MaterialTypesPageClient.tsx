...
export default function MaterialTypesPageClient({
  initialMaterials,
  initialError,
}: {
  initialMaterials: MaterialCatalogRow[]
  initialError?: string | null
}) {
  const [materials, setMaterials] = useState<MaterialCatalogRow[]>(initialMaterials)
  ...
  // Full reload after an Excel import - unlike the single-row mutations
  // below, an import can touch an unbounded number of rows, so there's no
  // sensible local patch to apply instead.
  async function loadMaterials() {
    try {
      const data = await getMaterialCatalog(false)
      setMaterials(data)
    } catch (error) {
  ...
  function openEditModal(material: MaterialCatalogRow) {
  ...
  function handleReactivate(material: MaterialCatalogRow) {
    startTransition(async () => {
      try {
