...
export default function SignatureSettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<SignatureDocumentType>('purchase_request')
  ...
  async function load(documentType: SignatureDocumentType) {
    ...
    try {
      ...
      setSlots(data.map((s) => ({ _key: s.id, label: s.label, system_key: s.system_key, signature_url: s.signature_url })))
    } catch (error) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        ...
        setSlots(saved.map((s) => ({ _key: s.id, label: s.label, system_key: s.system_key, signature_url: s.signature_url })))
