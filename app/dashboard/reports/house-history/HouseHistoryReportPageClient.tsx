...
// The `note` column exists on `billings` but the generated select type omits
// it (the select string here is long enough that supabase-js's inference
// falls back to a partial shape) - added back explicitly.
type PlotHistoryBill = Awaited<ReturnType<typeof getPlotHistoryReport>>[number] & { note?: string | null }
...
type CardEntry = {
  ...
  subtitle: string
  projectName: string
  records: PlotHistoryRecord[]
  total: number
}

export default function HouseHistoryReportPageClient({
  initialProjects,
  initialRows,
}: {
  initialProjects: Project[]
  initialRows: PlotHistoryBill[]
}) {
  ...
  const [rows, setRows] = useState<PlotHistoryBill[]>(initialRows)
  ...
  useEffect(() => {
    ...
    getPlotsByProjectId(projectId).then((list) => {
      setPlots((list || []).slice().sort((a, b) => collator.compare(a?.name || '', b?.name || '')))
    })
  }, [projectId, collator])
  ...
  const openHouseHistory = (card: CardEntry) => {
