...
type ExtraWorkRow = Awaited<ReturnType<typeof getExtraWorkReport>>[number]
type ReportRow = ExtraWorkRow & {
  _row_key: string
  _plot_name: string
  _billing_adjustments_for_plot: ExtraWorkRow['billing_adjustments']
  _dc_net_amount: number
}
...
export default function DCHistoryReportPageClient({
  initialProjects,
  initialData,
}: {
  initialProjects: Awaited<ReturnType<typeof getBillingOptions>>['projects']
  initialData: ExtraWorkRow[]
}) {
  const [projects] = useState<Project[]>(initialProjects)
  const [plots, setPlots] = useState<Plot[]>([])
  const [filters, setFilters] = useState<Filters>({})
  const [data, setData] = useState<ExtraWorkRow[]>(initialData)
  const [loading, setLoading] = useState(false)
  const [selectedGroupKey, setSelectedGroupKey] = useState<string | null>(null)
  const collator = useMemo(() => new Intl.Collator('th', { numeric: true, sensitivity: 'base' }), [])

  // Genuine client-side cascade: the plot dropdown only makes sense once a
  // project is chosen, so it fetches on demand rather than up front.
  useEffect(() => {
  ...
  const reportRows = useMemo(() => {
    ...
    for (const bill of sortedData) {
      ...
      const byPlot = new Map<string, typeof adjustments>()
      ...
      for (const [plotName, plotAdjustments] of byPlot.entries()) {
        const addTotal = plotAdjustments
          ...
          .reduce((sum: number, a) => sum + (a.quantity || 0) * (a.unit_price || 0), 0)
  ...
  const totalsByPlot = useMemo(() => {
    const totals = new Map<string, { key: string; project: string; plot: string; value: number; rows: ReportRow[] }>()
              ...
              {loading ? (
                ...
                reportRows.map((row) => (
                  <tr key={row._row_key} className="hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedGroupKey(`${row.projects?.name || 'ไม่ระบุโครงการ'}|||${row._plot_name || row.plots?.name || 'ไม่ระบุแปลง'}`)}>
                    <td className="px-4 py-3 text-slate-500">{new Date(row.billing_date || row.created_at || 0).toLocaleDateString('th-TH')}</td>
                ...
                <div key={row.id} className="rounded-lg border p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{new Date(row.billing_date || row.created_at || 0).toLocaleDateString('th-TH')}</div>
                  ...
                  <div className="mt-3 overflow-x-auto rounded-lg border border-slate-200">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {(row._billing_adjustments_for_plot || row.billing_adjustments || []).map((adj) => {
