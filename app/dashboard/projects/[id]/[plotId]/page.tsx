...
export default function PlotDetailPage() {
  ...
  // --- Helpers Update State แบบ Manual (ไม่ต้องโหลดหน้าใหม่) ---
  const updateJobWithNewPayment = (jobId: string, newPayment: any) => {
    // 1. อัปเดตในรายการ Jobs ทั้งหมด
    setJobs(prevJobs => prevJobs.map(job => {
        if (job.id === jobId) {
            const currentPayments = job.payments || []
            return { ...job, payments: [newPayment, ...currentPayments] } // เอาอันใหม่แทรกไปบนสุด
        }
        return job
    }))

    // 2. อัปเดตใน Modal (selectedJob)
  ...
  const handleAssign = (jobId: string, contractorId: string) => {
    ...
    startTransition(async () => { await assignContractor(jobId, contractorId, plotId, projectId) })
  }

  const handleStatusChange = (jobId: string, newStatus: string) => {
    setJobs(prev => prev.map(j => j.id === jobId ? { ...j, status: newStatus } : j))
    startTransition(async () => { await updateJobStatus(jobId, newStatus, plotId, projectId) })
  }
  ...
  const openPaymentModal = (job: any) => {
    // เรียง payment ให้ล่าสุดอยู่บน
    const sortedJob = {
        ...job,
        payments: job.payments?.sort((a:any, b:any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) || []
    }
  ...
  const handlePayRemaining = () => {
      ...
      const remainingPercent = (remaining / totalBoq) * 100
      setPayPercent(remainingPercent.toFixed(2))
      setNote(`ปิดงวดงานสุดท้าย (100%)`)
  }

  // [UPDATED] กดบันทึกแล้วหน้าไม่รีเฟรช Modal ไม่ปิด
  const handlePaymentSubmit = async (formData: FormData) => {
    setIsSubmittingPayment(true) // หมุนติ้วๆ
    ...
    try {
        // เรียก Server Action และรอรับ Data กลับมา
        ...
        if (newPayment) {
            ...
            // เคลียร์ช่องกรอก ให้พร้อมกรอกงวดถัดไป (ถ้ามี)
            setPayAmount('')
            setPayPercent('')
            setNote('')
        }
        ...
        // ❌ ไม่สั่งปิด Modal (setIsPaymentModalOpen(false)) แล้ว
    }
  }
  ...
  const getJobFinancials = (job: any) => {
    if (!job) return { totalBoq: 0, paid: 0, remaining: 0, paidPercent: 0 }
    
    const totalBoq = (job.boq_master?.quantity || 0) * (job.boq_master?.price_per_unit || 0)
    // คำนวณยอดจ่ายจาก State ปัจจุบัน (ซึ่งอัปเดตแล้ว)
    ...
    return { totalBoq, paid, remaining, paidPercent }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        <button onClick={() => router.push(`/dashboard/projects/${projectId}`)} className="text-sm text-slate-500 hover:text-indigo-600 w-fit flex gap-1 items-center">
          <ArrowLeft className="h-4 w-4" /> กลับไปหน้าโครงการ
        </button>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             <button onClick={handleSync} disabled={isPending} className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg border border-indigo-200 transition">
               <RefreshCw className={`h-3 w-3 ${isPending ? 'animate-spin' : ''}`} /> ดึง BOQ
             </button>
             <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-sm font-bold border border-slate-200">งานทั้งหมด {jobs.length} รายการ</span>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {jobs.map((job) => {
                const { totalBoq, paid } = getJobFinancials(job)
                    ...
                    <td className="px-4 py-3">
                        <div className="relative">
                            ...
                            <select 
                                ...
                                className={`w-full pl-7 pr-2 py-1.5 rounded border text-xs cursor-pointer outline-none ${job.contractor_id ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200'}`}
                            >
      ...
      <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title={`จัดการงวดงาน: ${selectedJob?.boq_master?.item_name || ''}`}>
         {selectedJob && (() => {
             const { totalBoq, paid, remaining, paidPercent } = getJobFinancials(selectedJob)
                 ...
                 <div className="space-y-5">
                    ...
                    <form action={handlePaymentSubmit} className="space-y-4">
                        ...
                        <div className="flex justify-end gap-2 pt-2 border-t">
                            <button type="button" onClick={() => setIsPaymentModalOpen(false)} className="btn-secondary text-sm">ปิด</button>
                            <button type="submit" disabled={isSubmittingPayment} className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded text-sm hover:bg-emerald-700 shadow-sm disabled:opacity-50">
