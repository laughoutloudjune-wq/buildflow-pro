-- Lets a single purchase-order LINE carry its own project/plot, separate
-- from the PO's own. Ordering from one supplier often means throwing in a
-- few materials that are low in stock for a different job (or for no job at
-- all yet - general/central stock) alongside the main order; today that can
-- only be recorded as a second PO, which defeats ordering together.
--
-- NULL on all three new columns means "inherit the PO's own project_id/
-- plot_id/plot_group_id" - today's behavior, unchanged for every existing
-- row and every line nobody touches. Setting project_id means the line
-- stands entirely on its own: plot_id/plot_group_id are then optional
-- *within that line's own project*, not layered on top of the PO's plot.
-- Unlike the PO-level rule, a line override does NOT require a plot/group -
-- picking just a project (e.g. central stock) is valid on its own.
--
-- Five functions restated in full per this repo's convention (Postgres has
-- no partial function replace):
--   po_create                   <- 202609170003_po_closes_request_line.sql
--   po_update                   <- 202609170003_po_closes_request_line.sql
--   goods_receipt_create        <- 202609220001_goods_receipt_pr_status_fix.sql
--   boq_control_rollup          <- 202609170004_drop_quantity_fulfilled.sql
--   boq_control_material_detail <- 202609170004_drop_quantity_fulfilled.sql
-- boq_control_unassigned is deliberately left untouched - it's a legacy
-- safety net for data that predates po_create's plot-scope validation, and
-- per-line overrides don't interact with it.
...
alter table public.purchase_order_items
  drop constraint if exists purchase_order_items_line_scope_check;
alter table public.purchase_order_items
  add constraint purchase_order_items_line_scope_check
  check (not (plot_id is not null and plot_group_id is not null));

-- ---------------------------------------------------------------------------
-- po_create: restated in full from 202609170003, adding per-item scope.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- po_update: restated in full from 202609170003, adding per-item scope.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- goods_receipt_create: restated in full from 202609220001, posting each
-- received line's stock movement against its OWN effective scope (the
-- item's override when it has one, else the PO's own project/plot) instead
-- of the PO's project/plot for every line uniformly.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select purchase_request_id, project_id, plot_id, plot_group_id, order_date
  into v_pr_id, v_project_id, v_plot_id, v_plot_group_id, v_order_date
...
-- ---------------------------------------------------------------------------
-- boq_control_rollup: restated in full from 202609170004. The `ordered` CTE
-- splits into two branches unioned together: the existing PO-level-weighted
-- logic (now restricted to non-overridden items), and a new branch for
-- overridden items, each with its own definite scope resolved the same way
-- a whole PO's scope already is (single plot = weight 1; plot group = spread
-- across its member plots, same as a group-scoped PO).
-- ---------------------------------------------------------------------------
...
as $$
  ...
  ordered_overridden as (
    select
      poi.material_type_id,
      sum(poi.quantity_ordered * ow.scope_plots_count::numeric / ow.total_plots) as ordered_qty,
      sum(poi.quantity_received * ow.scope_plots_count::numeric / ow.total_plots) as received_qty,
      sum(poi.quantity_ordered * poi.unit_price * ow.scope_plots_count::numeric / ow.total_plots) as ordered_value,
      sum(poi.quantity_received * poi.unit_price * ow.scope_plots_count::numeric / ow.total_plots) as received_value,
...
-- ---------------------------------------------------------------------------
-- boq_control_material_detail: restated in full from 202609170004. po_rows
-- splits the same way - existing PO-weighted rows restricted to
-- non-overridden items, plus a new set of rows for items overridden into
-- this project, labelled to say they came from a PO whose own scope is
-- elsewhere.
-- ---------------------------------------------------------------------------
