-- po_update previously blocked editing outright the moment any line had a
-- received quantity - necessary given it used to blindly
-- `delete from purchase_order_items` and reinsert everything from scratch,
-- which would either silently wipe quantity_received (if goods_receipt_items
-- didn't reference the old row) or hit a foreign-key violation (since it
-- does). Real usage often differs from what was ordered (short shipments,
-- etc.), so the buyer needs to revise the PO afterward and resend it to the
-- supplier - today that's impossible even just to lower the ordered
-- quantity down to match what actually arrived.
--
-- Items are now matched to their existing row by id (carried through from
-- the client for lines that already exist) instead of being wiped
-- wholesale: existing rows are UPDATEd in place (preserving
-- quantity_received and the goods_receipt_items FK), brand-new lines
-- (no id) are INSERTed, and an existing unreceived line dropped from the
-- payload is deleted - exactly matching the old behavior for a PO with no
-- receipts yet. A line that already has a received quantity can still be
-- edited (price, description, or bumping quantity_ordered up), but it can
-- neither be removed from the payload nor have its quantity_ordered set
-- below what was actually received - both would leave the PO disagreeing
-- with real, already-posted stock movements.
