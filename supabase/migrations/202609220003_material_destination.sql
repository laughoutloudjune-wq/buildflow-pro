-- Material Flow Phase 1 (MATERIAL_FLOW_PLAN.md) - the actual fix for the
-- phantom stock count. goods_receipt_create has only ever asked "which
-- project?" and posted every delivery into the one shared pool, even when
-- the truck drove straight to a house and unloaded there. Nothing ever drew
-- that material back down, which is why 104 materials sit on the books with
-- ~12,374 units that almost certainly aren't in the yard.
--
-- This adds the missing third question - where did it physically unload -
-- as `destination` ('store' | 'site') on the receipt (header default + a
-- per-line override, same convention as 202609220002's PO-line scope
-- override). `store` behaves exactly as today. `site` posts the existing
-- 'in' immediately followed by an 'out' (source_type = 'direct_to_site'),
-- carrying the same project/plot/batch, so:
--   - "what did this house consume" stays one rule - all 'out' movements,
--     whatever their source_type;
--   - the delivery still shows up in the material's movement history;
--   - the 'in' always lands before the 'out', so the negative-balance guard
--     inside _stock_movement_post can never reject a legitimate receipt.
--
-- `intended_destination` on the PO line is order-time metadata only - it
-- pre-fills the receipt's destination toggle and is never read by
-- goods_receipt_create. NULL means "not decided yet", same as every other
-- nullable field here.
--
-- Three functions restated in full per this repo's convention (Postgres has
-- no partial function replace):
--   po_create            <- 202609220002_po_item_project_plot_override.sql
--   po_update             <- 202609220002_po_item_project_plot_override.sql
--   goods_receipt_create  <- 202609220002_po_item_project_plot_override.sql
...
-- NULL = inherit the receipt's default_destination.
...
-- ---------------------------------------------------------------------------
-- po_create: restated from 202609220002, adding intended_destination on the
-- per-item insert. Nothing else changes.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- po_update: restated from 202609220002, adding intended_destination on both
-- the update-in-place and brand-new-line insert. Nothing else changes.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- goods_receipt_create: restated from 202609220002. Adds default_destination
-- on the receipt header and destination on each receipt line; resolves the
-- effective destination as coalesce(line, header) and, when it's 'site',
-- posts a matching 'out' (source_type = 'direct_to_site') right after the
-- existing 'in' for that line, same project/plot/batch.
-- ---------------------------------------------------------------------------
