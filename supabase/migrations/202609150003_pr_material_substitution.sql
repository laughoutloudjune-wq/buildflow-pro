-- The PO form already detects and displays a material substitution: when a
-- line prefilled from a purchase request has its material changed (a brand
-- swap, an out-of-stock substitute), PurchaseOrderForm.tsx's RequestLinkNote
-- shows "ตัดยอดแทน: <original material>" instead of "ตัดยอดจากคำขอ:" - the
-- purchase_request_item_id link is kept either way, so the request's
-- quantity still settles correctly. What was missing: the request's own
-- record (purchase_request_items.material_type_id) never actually updated,
-- so re-opening that PR later still shows the stale original material - the
-- PR's history quietly disagrees with what was really bought.
--
-- Fix: after a PO's items are written, any PO item carrying a
-- purchase_request_item_id whose material differs from that request line's
-- current material overwrites the request line's material to match. The
-- request becomes "what was actually ordered", not "what was originally
-- asked for" - which also keeps boq_control's per-PR check
-- (getBoqCheckForPurchaseRequest) comparing the right material against BOQ
-- for anyone who opens that request later.
--
-- Known limitation, accepted deliberately: if one request line's quantity
-- is later split across two POs using two different materials, the
-- request's single material_type_id column can only reflect the most
-- recent substitution, not "20 bags of X, 30 of Y". Quantity tracking is
-- unaffected either way - only the material label on the request line -
-- and splitting one line's fulfilment across two different materials is
-- rare enough that this is the right trade-off over adding new schema for
-- a case that mostly doesn't happen.
--
-- po_create/po_update are restated in full again (Postgres has no partial
-- function replace) from 202609150002_boq_qty_control_guardrails.sql,
-- adding only the one update statement, placed right after each function's
-- purchase_order_items writes.
...
begin
  ...
  -- Material substitution sync - see migration header comment.
