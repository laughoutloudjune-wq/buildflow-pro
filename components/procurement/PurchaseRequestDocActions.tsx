...
// server-rendered HTML (lib/pdf/purchaseRequestHtml.ts) - the preview asks
// for ?format=png, the download gets the PDF. Mirrors PurchaseOrderDocActions.
export default function PurchaseRequestDocActions({ requestId, prNo }: { requestId: string; prNo: number }) {
  const base = `/api/procurement/requests/${requestId}/pdf`
  const pngUrl = `${base}?format=png`
  const docLabel = `PR-${String(prNo).padStart(4, '0')}`
  ...
  return (
    <>
      ...
      <a href={`${base}?download=1`} download={`${docLabel}.pdf`}>
        <Button type="button" variant="secondary" size="sm">
